...
hiscore_dlist:
	dta $70, $70, $70	; Top border 3x8 blank

	dta $46, .lo(hiscore_txt), .hi(hiscore_txt) ; Hi score display

	dta $06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06,$06

	dta $F0			; Blank.i before defcon display

	dta $46,$02,$07		; Blank 6 @ $0702
...
temp:		.ds 1		; Temporary spot
p1scr_store:	.ds 6		; Score store (because we have to transform it)
...
hiscore:
	...
	ldx #$05
	lda #$00
hic:	sta p1scr_store,x
	dex
	bpl hic
	
	;; Transform hiscore

	ldx #$05		; 6 onscreen digits
hitl:	lda p1scr,x		; load next digit
	beq hitln		; if 0, skip.
	sec			; otherwise set up for subtraction
	sbc #$40		; subtract $40 (64) to get standard screen code
	sta p1scr_store,x	; store in p1scr_store,x
hitln:	dex			; to next digit
	bpl hitl		; continue if we've done offset 0.

	ldy VVBLKI_STORE	; temporarily swap back in OS immediate VBI
	ldx VVBLKI_STORE+1	; so that things work
	lda #$06
	jsr setvbv

	;; At this point, colors get reset to OS, so we...reset them :)
...
;;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	;; Pulled this in from main, we need it here ahead of time.
        ldy #$F8
        ldx #$A5
        lda #$06
        jsr SETVBV
...
HSCROF:
	...
	;; Store Immediate VBI so we can swap it in/out during hiscore.
	;; also zero out stuff at 0740.
hiscore_init:
	ldx #$1F
	lda #$00
gclr:	sta $0740,X
