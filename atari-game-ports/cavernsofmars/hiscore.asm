	;; FujiNet Hiscore Enabled for Caverns of Mars
...
p1scr_store:	.ds 7		; hiscore screen code storage
slot:	        .ds 1           ; Hiscore slot
scootslot:		.ds 1		; Another temp location
xoff:		.ds 1		; Score digit offset
tmp:	.ds 1			; Temporary spot
vkeybd_store:	.ds 2		; Store VKEYBD
	
hiscore:

	;; Restore keyboard so we can use it
	lda VKEYBD_STORE
	sta VKEYBD
	lda VKEYBD_STORE+1
	sta VKEYBD+1
	
	;; transform score to screen codes
...
hsds:	
	lda #$3e		; Switch to normal playfield
	sta SDMCTL
...
HFSLT3:	INY			; Increment current hiscore slot ptr.
	...
	CPY #$07		; Are we done with string comparison?
...
HSBL1:	LDA $13			; Check every 256 frames
	...
	lda #$3F		; Switch back to wide playfield
	sta SDMCTL
...
bh	jsr L1787
	...
	pla
	pla	
	jmp $0E68		; copied from original code.
